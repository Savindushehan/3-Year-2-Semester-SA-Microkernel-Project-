...
//    }
    ...
    private void openAddStockDialog() {
     ...
     // Division (ComboBox)
        ...
        // Item Name (ComboBox)
        createLabel(dialogShell, "Item Name:");
        Combo itemNameCombo = new Combo(dialogShell, SWT.DROP_DOWN | SWT.READ_ONLY);
        itemNameCombo.setLayoutData(textFieldGridData);

        // Category (ComboBox)
        createLabel(dialogShell, "Category:");
        Combo categoryCombo = new Combo(dialogShell, SWT.DROP_DOWN | SWT.READ_ONLY);
        categoryCombo.setLayoutData(textFieldGridData);
            ...
            public void widgetSelected(SelectionEvent e) {
                try {
                    ...
                    if (existingStocks.isEmpty()) {
                        ...
                        Stock existingStock = existingStocks.get(0); // Assuming there's only one entry per item
                        int updatedQuantity = Math.max(existingStock.getQuantity() - quantity, 0); // Prevent negative quantity

                        existingStock.setQuantity(updatedQuantity);
                      //  stockservice.updateStock(existingStock);
                        stockservice.updatstockwithshare(existingStock);
                    }
